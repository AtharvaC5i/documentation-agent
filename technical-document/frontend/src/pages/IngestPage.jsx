...
// ── Metric card (inline, no external component needed) ────────────────────────
...
// ✅ EXACT same logic as original — reads state.analysis and state.filteredFileCount
function AnalysisResult({ analysis, filteredFileCount, onContinue, onReset }) {
  const loc = analysis?.total_loc ?? 0;
  const strategy = loc > 50000 ? "RAPTOR" : "Flat"; // ← original logic preserved
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-4">
        <div className="grid grid-cols-4 divide-x divide-gray-100">
          ...
          <MetricCell
            ...
            sub="detected"
          />
          <MetricCell label="Strategy" value={strategy} sub="auto-selected" />
          <MetricCell
            label="Files"
            value={filteredFileCount}
            sub="after filter"
          />
        </div>
      </div>
          ...
          <div key={label} className="flex items-center gap-3">
            <span
              className="text-[10px] font-bold uppercase tracking-widest
                             text-gray-400 w-20 flex-shrink-0"
            >
              {label}
            </span>
            <TagRow items={items} />
...
export default function IngestPage() {
  ...
  // ── Same simulate logic as original ──────────────────────────────────────
  ...
  // ── GitHub submit — EXACT same logic, just cleaned up ────────────────────
    ...
    try {
      ...
      dispatch({
        ...
        payload: {
          ...
          analysis: data.analysis, // ← nested analysis object
  ...
  return (
    <div className="flex gap-10">
      ...
      <div className="flex-[6] min-w-0">
            ...
            <div className="mb-4">
              ...
              <label
                className={`flex flex-col items-center justify-center gap-2
                                 border-2 border-dashed rounded-xl px-6 py-8
                ...
                {zipFile ? (
                  <>
                    ...
                    <span className="text-xs text-gray-400">
                      {(zipFileSize / 1024 / 1024).toFixed(1)} MB — click to
                      change
                    </span>
                  </>
                ) : (
                  <>
                    ...
                    <span className="text-sm text-gray-500 font-medium">
                      Click to select a <code className="font-mono">.zip</code>{" "}
                      file
                    </span>
                  </>
                )}
        ...
        {/* ── Analysis result — reads state.analysis + state.filteredFileCount ── */}
        {hasResult && !loading && (
          <AnalysisResult
      ...
      <div className="flex-[4] min-w-0 hidden md:block">
        ...
        {/* aligns with form start below heading */}
